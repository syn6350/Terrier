...
<div id="content" class="col-md-10 nav_and_content">
	<div id="title">
		<span class="glyphicon glyphicon-send"></span>&nbsp;요청 대기 리스트
	</div>
	...
	<div class="text-center">
		<ul class="pagination">
			...
			<c:if test="${pageMaker.prev}">
				<c:choose>
				    <c:when test="${uri[2] eq 'w_list'}">
				    	<li><a href="w_list${pageMaker.makeSearch(pageMaker.startPage -1)}">&laquo;</a></li>
				    </c:when>
					
					<c:otherwise>
						<li><a href="w_list_search${pageMaker.makeSearch(pageMaker.startPage -1)}${send_data}">&laquo;</a></li>
					</c:otherwise>
				</c:choose>
			</c:if>
			...
			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				<li
					...
					<c:choose>
						<c:when test="${uri[2] eq 'w_list'}">
							<a href="w_list${pageMaker.makeSearch(idx)}">${idx}</a><!-- 페이지를 누를때마다 계속 uri요청을 하여 리바인딩하는형식 -->
						</c:when>
						
						<c:otherwise>
							<a href="w_list_search${pageMaker.makeSearch(idx)}${send_data}">${idx}</a><!-- 페이지를 누를때마다 계속 uri요청을 하여 리바인딩하는형식 -->
						</c:otherwise>
					</c:choose>
				</li>
			</c:forEach>
			...
			<c:if test="${pageMaker.next && pageMaker.endPage>0}">
				<c:choose>
						<c:when test="${uri[2] eq 'w_list'}">
							<li><a href="w_list${pageMaker.makeSearch(pageMaker.endPage+1)}">&raquo;</a></li>
						</c:when>
						
						<c:otherwise>
							<li><a href="w_list_search${pageMaker.makeSearch(pageMaker.endPage+1)}${send_data}">&raquo;</a></li>
						</c:otherwise>
				</c:choose>
			</c:if>		
...
$("#search_button").on("click",function(){//검색 활성화	
	formObj.attr("action","w_list_search");
	formObj.attr("method","get");
	formObj.submit();
});

function read_request(bno) {
	location.href="w_read?bno="+bno;
}
</script>